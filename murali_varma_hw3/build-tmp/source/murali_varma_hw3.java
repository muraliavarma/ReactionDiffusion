...
public void mousePressed() {
	...
	cellDetails = "Cell[" + yCell + "][" + xCell + "]: u = " + String.format("%.3f", u[yCell][xCell]) + ", v = " + String.format("%.3f", v[yCell][xCell]) +
		(pMode == VARYING_PARAMETER_MODE ? ", k = " + String.format("%.3f", (0.03f + ((0.04f * xCell) / NUM_HORIZONTAL_CELLS))) +
		", f = " + String.format("%.3f", (0.08f * (1.0f - (1.0f * yCell / NUM_VERTICAL_CELLS)))) : "");
	println(cellDetails);
	drawControls();
}
...
public void swapMode(String type) {
	if (type == "rd") {
		if (rdMode == RD_MODE) {
			rdMode = D_MODE;
		}
